...
/// {@link io.github.qlivedev.runtime.scalar.ConditionCoercing}'s business, and it converts every value in
...
public class ConditionTransformer
{
    ...
    private Condition condition(CNode node)
    {
        return switch (node)
        {
            ...
            case io.github.qlivedev.model.condition.Condition c ->
    ...
    private Condition logic(io.github.qlivedev.model.condition.Condition node)
    {
    ...
    private Condition comparison(io.github.qlivedev.model.condition.Condition node)
    {
        final List<ExistsScope> scopes = new ArrayList<>();
    ...
    private Field<?> field(CNode node, DataType<?> hint, List<ExistsScope> scopes)
    {
        return switch (node)
        {
            ...
            case io.github.qlivedev.model.condition.Field f ->
            {
