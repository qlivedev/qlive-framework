/*
 * Generates src/styles/qlive.css from the six brand colors.
 *
 * Every derived value is a mix of two palette colors -- never a mix with
...
/*
 * How two palette colors are blended.
 *
 ...
 * social icons, theme select -- is colored against the bar rather than
 ...
 * Change the six colors or the contrast target there and run \`pnpm palette\`.
 *
 *   ${INK}  ink      ${CREAM}  cream
 *   ${ORANGE}  orange   ${AMBER}  amber   ${SALMON}  salmon   ${TEAL}  teal
 *
...
:root {
    ...
    /* Starlight colors prose links with this and would otherwise take the raw
...
/* The bar carries its own color, so what sits on it is measured against the
...
/*
 * An Inkscape palette, so a diagram drawn by hand uses these colors and not
 * approximations of them.
 *
 * Besides the six brand colors it carries a "both themes" set: a single SVG
...
const gpl = [
    ...
    "#",
    "# The six brand colors, then colors safe on both the light and the dark",
    "# page. Use the second set for anything in a diagram: one SVG is served to",
    "# both themes, and no brand color clears 3:1 on both backgrounds.",
    "#",
    gplRow(INK, "ink"),
    gplRow(CREAM, "cream"),
